def solve():
    ...
    print(f"Debug: n={n}, T={T}, K={K}")
    print(f"Debug: weights={w[1:]}")
    print(f"Debug: Valid range=[{T-K}, {T+K}]")
    ...
    def dfs(u, parent):
        ...
        print(f"Debug: Node {u}, subtree_sum={subtree_sum[u]}, children={children}")
        
        # Option 1: Keep entire subtree together
        if T - K <= subtree_sum[u] <= T + K:
            min_cuts[u] = 0
            print(f"Debug: Node {u} subtree is balanced, cuts=0")
        
        # Option 2: Cut children and keep only this node
        if T - K <= w[u] <= T + K and children:
            total_child_cuts = 0
            ...
            if all_children_solvable:
                ...
                print(f"Debug: Node {u} cutting children, cuts={cuts_needed}")
        
        print(f"Debug: Final min_cuts[{u}]={min_cuts[u]}")
